...
if __name__ == "__main__":
    ...
    # Print the results
    time_route1, _ = calculate_transmission_time(route1, CCA, CCA_PROBABILITY_FAILURE, PACKET_SIZE, GUARD_TIME)
    print(f"Transmission time for route {route1}: {time_route1:.2f} ms")

    time_route2, _ = calculate_transmission_time(route2, CCA, CCA_PROBABILITY_FAILURE, PACKET_SIZE, GUARD_TIME)
    print(f"Transmission time for route {route2}: {time_route2:.2f} ms")

    time_route3, _ = calculate_transmission_time(route3, CCA, CCA_PROBABILITY_FAILURE, PACKET_SIZE, GUARD_TIME)
